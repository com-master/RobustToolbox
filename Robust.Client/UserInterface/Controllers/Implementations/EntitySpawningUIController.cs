...
public sealed partial class EntitySpawningUIController : UIController
{
    ...
    private static bool DoesEntityMatchSearch(EntityPrototype prototype, string searchStr)
    {
        ...
        if (prototype.ID.ContainsYoInsensitive(searchStr, StringComparison.InvariantCultureIgnoreCase))
            return true;

        if (prototype.EditorSuffix != null &&
            prototype.EditorSuffix.ContainsYoInsensitive(searchStr, StringComparison.InvariantCultureIgnoreCase))
            return true;

        if (string.IsNullOrEmpty(prototype.Name))
            return false;

        if (prototype.Name.ContainsYoInsensitive(searchStr, StringComparison.InvariantCultureIgnoreCase))
            return true;
